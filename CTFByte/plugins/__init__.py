...
from CTFByte.utils.decorators import admins_only as admins_only_wrapper
from CTFByte.utils.plugins import override_template as utils_override_template
...
from CTFByte.utils.plugins import register_script as utils_register_plugin_script
from CTFByte.utils.plugins import register_stylesheet as utils_register_plugin_stylesheet
...
def register_plugin_assets_directory(app, base_path, admins_only=False, endpoint=None):
    ...
    :param app: A CTFByte application
...
def register_plugin_asset(app, asset_path, admins_only=False, endpoint=None):
    """
    Registers an file path to be served by CTFByte
...
def bypass_csrf_protection(f):
    ...
    :return: Returns a function with the _bypass_csrf attribute set which tells CTFByte to not require CSRF protection.
    """
    f._bypass_csrf = True
    return f
...
def init_plugins(app):
    """
    Searches for the load function in modules in the CTFByte/plugins folder. This function is called with the current CTFByte
    app as a parameter. This allows CTFByte plugins to modify CTFByte's behavior.
    ...
    if app.config.get("SAFE_MODE", False) is False:
        for plugin in get_plugin_names():
            module = "." + plugin
            module = importlib.import_module(module, package="CTFByte.plugins")
            module.load(app)
