...
const files = {
  upload: (form, extra_data, cb) => {
    ...
    $.ajax({
      url: CTFByte.config.urlRoot + "/api/v1/files",
      data: formData,
      type: "POST",
      cache: false,
      contentType: false,
...
const comments = {
  get_comments: extra_args => {
    const CTFByte = window.CTFByte;
    return CTFByte.fetch("/api/v1/comments?" + $.param(extra_args), {
      method: "GET",
  ...
  delete_comment: comment_id => {
    const CTFByte = window.CTFByte;
    return CTFByte.fetch(`/api/v1/comments/${comment_id}`, {
      method: "DELETE",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
