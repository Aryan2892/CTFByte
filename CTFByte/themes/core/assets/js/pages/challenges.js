...
const displayChal = chal => {
  return Promise.all([
    CTFByte.api.get_challenge({ challengeId: chal.id }),
...
function renderSubmissionResponse(response) {
  ...
  const next_btn = $(
    ...
    setTimeout(function() {
      loadChal(CTFByte._internal.challenge.data.next_id);
    }, 500);
  });
  ...
  if (result.status === "authentication_required") {
    ...
    if (CTFByte._internal.challenge.data.next_id) {
...
function getSolves(id) {
  return CTFByte.api.get_challenge_solves({ challengeId: id }).then(response => {
...
function loadChals() {
  return CTFByte.api.get_challenge_list().then(function(response) {
    const categories = [];
...
const displayUnlock = id => {
  ezQuery({
    ...
    success: () => {
      ...
      CTFByte.api.post_unlock_list({}, params).then(response => {
        if (response.success) {
          CTFByte.api.get_hint({ hintId: id }).then(response => {
