  ...
  CTFByte.fetch("/api/v1/teams", {
    ...
    .then(function(response) {
      if (response.success) {
        ...
        window.location = CTFByte.config.urlRoot + "/admin/teams/" + team_id;
      } else {
...
function updateTeam(event) {
  ...
  CTFByte.fetch("/api/v1/teams/" + window.TEAM_ID, {
    method: "PATCH",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
  ...
  ezQuery({
    ...
    success: function() {
      ...
      for (var subId of submissionIDs) {
        reqs.push(CTFByte.api.delete_submission({ submissionId: subId }));
      }
  ...
  ezQuery({
    ...
    success: function() {
      ...
      for (var awardID of awardIDs) {
        let req = CTFByte.fetch("/api/v1/awards/" + awardID, {
          method: "DELETE",
          credentials: "same-origin",
          headers: {
            Accept: "application/json",
      ...
      ezAlert({
        ...
        success: function() {
          ...
          for (var challengeID of challengeIDs) {
            ...
            let req = CTFByte.fetch("/api/v1/submissions", {
              method: "POST",
              credentials: "same-origin",
              headers: {
                Accept: "application/json",
...
$(() => {
  ...
  $(".invite-team").click(function(_e) {
    CTFByte.fetch(`/api/v1/teams/${window.TEAM_ID}/members`, {
      method: "POST",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
    ...
    ezQuery({
      ...
      success: function() {
        CTFByte.fetch("/api/v1/teams/" + window.TEAM_ID, {
          ...
          .then(function(response) {
            if (response.success) {
              window.location = CTFByte.config.urlRoot + "/admin/teams";
            }
          });
      }
    });
