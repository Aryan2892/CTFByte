  ...
  CTFByte.fetch(url, {
    ...
    .then(function(response) {
      if (response.success) {
        ...
        window.location = CTFByte.config.urlRoot + "/admin/users/" + user_id;
      } else {
...
function updateUser(event) {
  ...
  CTFByte.fetch("/api/v1/users/" + window.USER_ID, {
    method: "PATCH",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
  ...
  ezQuery({
    ...
    success: function() {
      CTFByte.fetch("/api/v1/users/" + window.USER_ID, {
        ...
        .then(function(response) {
          if (response.success) {
            window.location = CTFByte.config.urlRoot + "/admin/users";
          }
        });
    }
  });
...
function emailUser(event) {
  ...
  CTFByte.fetch("/api/v1/users/" + window.USER_ID + "/email", {
    method: "POST",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
