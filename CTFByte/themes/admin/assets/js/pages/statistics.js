...
const graph_configs = {
  "#solves-graph": {
    data: () => CTFByte.api.get_challenge_solve_statistics(),
    format: response => {
  ...
  "#keys-pie-graph": {
    data: () => CTFByte.api.get_submission_property_counts({ column: "type" }),
    format: response => {
      const data = response.data;
  ...
  "#categories-pie-graph": {
    data: () => CTFByte.api.get_challenge_property_counts({ column: "category" }),
    format: response => {
      const data = response.data;
    ...
    layout: annotations => ({
      ...
      yaxis: {
        title: "Percentage of {0} (%)".format(
          CTFByte.config.userMode.charAt(0).toUpperCase() +
            CTFByte.config.userMode.slice(1)
        ),
        range: [0, 100]
      },
      annotations: annotations
    }),
    data: () => CTFByte.api.get_challenge_solve_percentages(),
    format: response => {
      const data = response.data;
  ...
  "#score-distribution-graph": {
    ...
    data: () =>
      CTFByte.fetch("/api/v1/statistics/scores/distribution").then(function(
        response
      ) {
