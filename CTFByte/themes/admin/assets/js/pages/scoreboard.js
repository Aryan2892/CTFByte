...
const api_func = {
  users: (x, y) => CTFByte.api.patch_user_public({ userId: x }, y),
  teams: (x, y) => CTFByte.api.patch_team_public({ teamId: x }, y)
};
...
function toggleAccount() {
  ...
  api_func[CTFByte.config.userMode](id, params).then(response => {
    if (response.success) {
      if (hidden) {
...
function toggleSelectedAccounts(selectedAccounts, action) {
  ...
  for (let accId of selectedAccounts.accounts) {
    reqs.push(api_func[CTFByte.config.userMode](accId, params));
  }
