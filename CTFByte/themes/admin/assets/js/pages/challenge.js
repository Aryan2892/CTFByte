...
const loadHint = id => {
  CTFByte.api.get_hint({ hintId: id, preview: true }).then(response => {
...
function handleChallengeOptions(event) {
  ...
  let save_challenge = function() {
    CTFByte.fetch("/api/v1/challenges/" + params.challenge_id, {
      method: "PATCH",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
      ...
      .then(function(data) {
        if (data.success) {
          setTimeout(function() {
            window.location =
              CTFByte.config.urlRoot + "/admin/challenges/" + params.challenge_id;
          }, 700);
        }
      });
  };
...
$(() => {
  $(".preview-challenge").click(function(_e) {
    CTFByte._internal.challenge = {};
    $.get(
      CTFByte.config.urlRoot + "/api/v1/challenges/" + window.CHALLENGE_ID,
      function(response) {
        ...
        $.getScript(
          CTFByte.config.urlRoot + challenge_data.type_data.scripts.view,
          function() {
            const challenge = CTFByte._internal.challenge;
  ...
  $(".delete-challenge").click(function(_e) {
    ezQuery({
      ...
      success: function() {
        CTFByte.fetch("/api/v1/challenges/" + window.CHALLENGE_ID, {
          method: "DELETE"
        })
          .then(function(response) {
            return response.json();
          ...
          .then(function(response) {
            if (response.success) {
              window.location = CTFByte.config.urlRoot + "/admin/challenges";
            }
          });
      }
    });
  ...
  $("#challenge-update-container > form").submit(function(e) {
    ...
    CTFByte.fetch("/api/v1/challenges/" + window.CHALLENGE_ID + "/flags", {
      method: "GET",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
