...
export default {
  ...
  methods: {
    loadTags: function() {
      CTFByte.fetch(`/api/v1/challenges/${this.$props.challenge_id}/tags`, {
        method: "GET",
    ...
    addTag: function() {
      ...
      CTFByte.api.post_tag_list({}, params).then(response => {
        if (response.success) {
          this.tagValue = "";
          this.loadTags();
        }
    ...
    deleteTag: function(tag_id) {
      CTFByte.api.delete_tag({ tagId: tag_id }).then(response => {
        if (response.success) {
          this.loadTags();
        }
      });
