...
from CTFByte import __version__ as CTFD_VERSION
from CTFByte.cache import cache
from CTFByte.constants.themes import DEFAULT_THEME
from CTFByte.models import db, get_class_by_tablename
from CTFByte.plugins import get_plugin_names
from CTFByte.plugins.migrations import current as plugin_current
from CTFByte.plugins.migrations import upgrade as plugin_upgrade
from CTFByte.utils import get_app_config, set_config, string_types
from CTFByte.utils.dates import unix_time
from CTFByte.utils.exports.databases import is_database_mariadb
from CTFByte.utils.exports.freeze import freeze_export
from CTFByte.utils.migrations import (
    create_database,
    drop_database,
...
from CTFByte.utils.uploads import get_uploader
...
def import_ctf(backup, erase=True):
    ...
    if "db" not in member_dirs:
        ...
        raise Exception(
            'CTFByte couldn\'t find the "db" folder in this backup. '
    ...
    # Check if the alembic version is from CTFByte 1.x
    if alembic_version in (
        ...
        set_import_error(
            "Exception: The version of CTFByte that this backup is from is too old to be automatically imported."
        )
        raise Exception(
            "The version of CTFByte that this backup is from is too old to be automatically imported."
        )
    ...
    if sqlite is False and alembic_version not in get_available_revisions():
        set_import_error(
            "Exception: The target migration in this backup is not available in this version of CTFByte."
        )
        raise Exception(
            "The target migration in this backup is not available in this version of CTFByte."
        )
            ...
            if member.startswith("db/"):
                ...
                if data:
                    ...
                    for i, entry in enumerate(saved["results"]):
                        ...
                        # databases. Specifically between MySQL and MariaDB. Because CTFByte standardizes against MySQL
